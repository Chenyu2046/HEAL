...
class SimulatedGerrit:
    ...
    def submit(self, intent: SubmissionIntent, candidate: Candidate) -> SubmissionResponse:
        key = (intent.change_id, intent.fixed_commit)
        ...
        response = SubmissionResponse(
            ...
            patch_set="1",
            remote_revision=intent.fixed_commit,
            response_known=True,
        )
    ...
    def query(self, intent: SubmissionIntent) -> SubmissionResponse:
        return self._submissions.get((intent.change_id, intent.fixed_commit), SubmissionResponse(SubmissionStatus.SUBMISSION_UNKNOWN, "no matching simulated remote revision", response_known=True, safe_to_retry=True))
